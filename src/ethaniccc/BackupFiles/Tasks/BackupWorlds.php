...
class BackupWorlds extends AsyncTask{
    ...
    public function onRun(){
        $destination = 'plugin_data/BackupFiles/WorldBackup';
        if(file_exists($destination)) $this->delete_old_backup($destination);
        sleep(2);
        mkdir($destination);
        $this->backup('worlds', $destination);
    }

    public function onCompletion(Server $server){
        Server::getInstance()->getLogger()->info(TextFormat::GREEN . "Your worlds have been saved!");
    }
