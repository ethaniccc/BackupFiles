...
class BackupPluginData extends AsyncTask{
    ...
    public function onRun(){
        $destination = 'plugin_data/BackupFiles/PluginDataBackup';
        if(file_exists($destination)) $this->delete_old_backup($destination);
        sleep(2);
        mkdir($destination);
        $this->backup('plugin_data', $destination);
    }

    public function onCompletion(Server $server){
        Server::getInstance()->getLogger()->info(TextFormat::GREEN . "Your plugin data have been saved!");
    }
